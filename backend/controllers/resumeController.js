...
const OpenAI = require('openai');// backend/controllers/resumeController.js
const fs = require('fs');
const path = require('path');
...
const parseResume = async (req, res) => {
  try {
    ...
    // Step 2: Parse — Gemini first, fallback to manual
    let parsedData;
    let parseMethod = 'gemini';
    ...
    try {
      ...
      parsedData = await parseResumeWithGemini(resumeText);
      console.log('✅ Gemini parsing complete');
    } catch (aiError) {
      console.error('⚠️ Gemini failed:', aiError.message);
    ...
    res.json({
      success: true,
      message: `Resume parsed (${parseMethod === 'gemini' ? 'Gemini 2.5 Flash' : 'Keyword Matching'})`,
      data: parsedData,
      parseMethod,
      resumeFile: {
